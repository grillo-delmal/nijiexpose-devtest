...
if [ -z ${CHECKOUT_TARGET} ]; then
    if [ -z ${EXT_NIJI} ]; then
        CHECKOUT_TARGET=$(python3 ./scripts/find-nijiexpose-hash.py ./io.github.grillo_delmal.nijiexpose.yml)
    else
        CHECKOUT_TARGET=$(python3 ./scripts/find-nijiexpose-hash.py ${EXT_NIJI} ext)
    fi
fi
...
git -C ./nijiexpose/ checkout $CHECKOUT_TARGET 2>/dev/null
...
if [ "${NIGHTLY}" == "0" ]; then
    # Update repos to their state at nijiexpose commit date
    NIJI_DATE=$(git -C ./nijiexpose/ show -s --format=%ci)
    for d in ./deps/*/ ; do
...
mv ./dep.build/nijiexpose/dub.selections.json ./dep.build/nijiexpose/dub.selections.json.bak
jq ".versions += {\"semver\": \"$(semver ./dep.build/deps/semver)\", \"gitver\": \"$(semver ./dep.build/deps/gitver)\"}" \
    ./dep.build/nijiexpose/dub.selections.json.bak > ./dep.build/nijiexpose/dub.selections.json

# Generate the dependency file
python3 ./scripts/flatpak-dub-generator.py \
    --output=./dep.build/dub-dependencies.json \
    ./dep.build/nijiexpose/dub.selections.json
